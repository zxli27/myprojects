...
// func (rf *Raft) candidateCheck() {
	
// 	for {
// 		electionTimeout := rand.Intn(800)+800
// 		for (!time.Now().After(rf.startTime.Add(time.Millisecond*time.Duration(electionTimeout)))) {
// 			if rf.voteNum > rf.serverNum/2 && rf.state==2 {
// 				rf.state = 1
// 				rf.voteNum = 0
// 				go rf.heartbeat()
// 			}
...
// 		if (time.Now().After(rf.startTime.Add(time.Millisecond*time.Duration(electionTimeout)))) && rf.state != 1 {
...
// 				args.LastLogTerm = rf.entries[args.LastLogIndex].Term
